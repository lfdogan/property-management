...
//             var todayMonthText = getMonthText(this.today);
        ...
        function do_a (callback){
            setTimeout(function(){
                ...
                */
                console.log("today is",mdyy(todayMilliseconds));
                leases216554THSTREERef.once("value", function(snapshot) {
                    snapshot.forEach(function(childSnapshot) {
                        ...
                        if (childData.leaseBegin < todayMilliseconds && todayMilliseconds < childData.leaseEnd){
                            //if lease has an early termination date, check that it hasn't occurred yet
                            if (!childData.earlyTermination || todayMilliseconds < childData.earlyTermination) {
                                ...
                                transactionsRef.once("value", function(snapshot) {
                                    snapshot.forEach(function(childSnapshot) {
                                        ...
                                        if (transactionschildData.account == 4000){
                                            ...
                                            if (todayMonthIndex == new Date(transactionschildData.payDate).getMonth() && todayYearIndex == new Date(transactionschildData.payDate).getFullYear() && transactionschildData.payDate < todayMilliseconds){
                ...
                */
                for (var i=0; i < buildingData.length; i++) {
                    //console.log("received as of "+mdyy(todayMilliseconds)+": $"+buildingData[i].received);
        ...
        function do_b () {
            //console.log( '`do_b`: now we can make sure `do_b` comes out after `do_a`' );
             ...
             var chart = new CanvasJS.Chart("chartContainer", {
                 ...
                 title:{text: todayMonthText + " Rent Received"},  
                 axisY:{
                    title: "percentage", // label on left side of graph
                    //valueFormatString: "#0.#,.",
                },
                 data: [
                     {
                         ...
                         yValueFormatString: "$#,###,###", // format of data on hover
